...
- **종합부동산세** (`jongbuse.html`) — 12억/9억 공제·공정시장가액비율 60%·일반/중과 세율·재산세 중복공제·고령자·장기보유 세액공제·농특세
...
├─ jongbuse.html          # 종합부동산세(종부세) 계산기 + 해설
...
├─ tests-jongbu.html      # 종부세 골든테스트 (noindex) — 39 PASS
...
└─ js/
   ...
   ├─ rules-jongbu-2026.js   # 종부세 세율·공제 데이터
   ├─ jongbu-tax.js          # 종부세 순수 엔진
   ├─ app-jongbu.js          # 종부세 UI + localStorage
...
- **종합부동산세** (2026-06-13): 종부세법 제8·9조 세율(2주택 이하 0.5~2.7%·3주택 이상 0.5~5.0%, 누진공제 역산)·기본공제(12억/9억)·공정시장가액비율 60%(2026 유지)·고령자/장기보유 세액공제(합산 80%)·농특세 20% 대조(국세청·웹검색). 재산세 중복분 공제는 시행령 제5조의3 공식+국세청 계산예시로 검산(1주택 정확, 다주택 근사). `tests-jongbu.html` **39 PASS**. 세부담상한(150%)·합산배제·부부공동명의·법인은 v1 미반영(화면 명시).
...
2. 애드센스 신청 (콘텐츠 7편 + 계산기 6개 + 개인정보처리방침. 색인 잡힌 뒤 신청).
3. 커뮤니티 홍보 (메모리의 채널 등급·maker 톤 원칙 따라).

## 다음 단계 (기획서 기준)
- 다음 계산기: 증여세 ✅ · 상속세 ✅(2026-06-12) · 종합부동산세(종부세) ✅(2026-06-13 완료). → 부동산 세금 생애주기(살때·가질때·팔때·물려줄때) 전 구간 커버 완성. 이후 후보: 부동산중개수수료(레드오션이라 보류) 등 (시장조사 메모리 참고)
